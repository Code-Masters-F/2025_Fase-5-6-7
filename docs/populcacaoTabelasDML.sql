...
INSERT INTO TRANSACAO_FIAT (conta_externa_id, conta_interna_id, valor, tipo, data_hora, cpf, codigo_banco_externo)
SELECT ce.id_conta_externa, ci.id_conta_interna, 10000, 'DEPOSITO', TIMESTAMP '2023-01-15 09:00:00', cl_ext.cpf, ce.codigo_banco_externo
...
INSERT INTO TRANSACAO_FIAT (conta_externa_id, conta_interna_id, valor, tipo, data_hora, cpf, codigo_banco_externo)
SELECT ce.id_conta_externa, ci.id_conta_interna, 15000, 'DEPOSITO', TIMESTAMP '2023-02-20 10:30:00', cl_ext.cpf, ce.codigo_banco_externo
FROM CONTA_EXTERNA ce
JOIN CLIENTE cl_ext ON ce.cliente_id_cliente = cl_ext.id_cliente
JOIN CONTA_INTERNA ci ON ci.cliente_id_cliente != cl_ext.id_cliente
JOIN CLIENTE cl_int ON ci.cliente_id_cliente = cl_int.id_cliente
...
INSERT INTO TRANSACAO_FIAT (conta_externa_id, conta_interna_id, valor, tipo, data_hora, cpf, codigo_banco_externo)
SELECT ce.id_conta_externa, ci.id_conta_interna, 8000, 'DEPOSITO', TIMESTAMP '2023-03-10 14:15:00', cl_ext.cpf, ce.codigo_banco_externo
FROM CONTA_EXTERNA ce
JOIN CLIENTE cl_ext ON ce.cliente_id_cliente = cl_ext.id_cliente
JOIN CONTA_INTERNA ci ON ci.cliente_id_cliente != cl_ext.id_cliente
JOIN CLIENTE cl_int ON ci.cliente_id_cliente = cl_int.id_cliente
...
INSERT INTO TRANSACAO_FIAT (conta_externa_id, conta_interna_id, valor, tipo, data_hora, cpf, codigo_banco_externo)
SELECT ce.id_conta_externa, ci.id_conta_interna, 5000, 'SAQUE', TIMESTAMP '2023-06-15 16:45:00', cl_int.cpf, ce.codigo_banco_externo
FROM CONTA_EXTERNA ce
JOIN CLIENTE cl_ext ON ce.cliente_id_cliente = cl_ext.id_cliente
JOIN CONTA_INTERNA ci ON ci.cliente_id_cliente != cl_ext.id_cliente
JOIN CLIENTE cl_int ON ci.cliente_id_cliente = cl_int.id_cliente
...
INSERT INTO TRANSACAO_FIAT (conta_externa_id, conta_interna_id, valor, tipo, data_hora, cpf, codigo_banco_externo)
SELECT ce.id_conta_externa, ci.id_conta_interna, 3000, 'SAQUE', TIMESTAMP '2023-07-20 11:20:00', cl_int.cpf, ce.codigo_banco_externo
FROM CONTA_EXTERNA ce
JOIN CLIENTE cl_ext ON ce.cliente_id_cliente = cl_ext.id_cliente
JOIN CONTA_INTERNA ci ON ci.cliente_id_cliente != cl_ext.id_cliente
JOIN CLIENTE cl_int ON ci.cliente_id_cliente = cl_int.id_cliente
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 0.25, 45000, TIMESTAMP '2024-01-15 09:30:00', 'CONCLUIDA', cl.cpf
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 1.5, 2800, TIMESTAMP '2024-01-15 10:45:00', 'CONCLUIDA', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'ETH'
WHERE cl.nome = 'Maria Oliveira Costa' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 500, 0.45, TIMESTAMP '2024-01-16 11:20:00', 'CONCLUIDA', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'ADA'
WHERE cl.nome = 'Pedro Ferreira Lima' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 5, 95.5, TIMESTAMP '2024-01-16 14:30:00', 'CONCLUIDA', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'SOL'
WHERE cl.nome = 'Ana Carolina Souza' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'VENDA', 0.1, 46000, TIMESTAMP '2024-01-18 16:20:00', 'CONCLUIDA', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'BTC'
WHERE cl.nome = 'João Silva Santos' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'VENDA', 0.5, 2850, TIMESTAMP '2024-01-19 13:45:00', 'CONCLUIDA', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'ETH'
WHERE cl.nome = 'Maria Oliveira Costa' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 300, 0.85, TIMESTAMP '2024-01-21 15:10:00', 'PENDENTE', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'MATIC'
WHERE cl.nome = 'Fernanda Almeida' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 20, 12.5, TIMESTAMP '2024-01-22 12:25:00', 'CONCLUIDA', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'LINK'
WHERE cl.nome = 'Ricardo Barbosa' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 8, 85, TIMESTAMP '2024-01-23 09:40:00', 'FALHOU', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'LTC'
WHERE cl.nome = 'Juliana Rodrigues' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 12, 6.25, TIMESTAMP '2024-01-24 14:55:00', 'CONCLUIDA', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'DOT'
WHERE cl.nome = 'Juliana Rodrigues' AND ROWNUM = 1;
...
INSERT INTO TRANSACAO_CRIPTOMOEDA (conta_interna_id, criptomoeda_id_criptomoeda, tipo_operacao, quantidade_criptomoeda, valor_unitario, data_hora, status, cpf)
SELECT ci.id_conta_interna, cr.id_criptomoeda, 'COMPRA', 25, 8.4, TIMESTAMP '2024-01-25 11:30:00', 'CANCELADA', cl.cpf
FROM CONTA_INTERNA ci
JOIN CLIENTE cl ON ci.cliente_id_cliente = cl.id_cliente
JOIN CRIPTOMOEDA cr ON cr.sigla = 'UNI'
WHERE cl.nome = 'Carlos Eduardo Pereira' AND ROWNUM = 1;
